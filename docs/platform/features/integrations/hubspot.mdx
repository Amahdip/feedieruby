---
title: "HubSpot"
description: "Learn how to integrate SalamRuby with HubSpot to automatically create or update contacts when survey responses are submitted."
---

<Note>
  SalamRuby doesn't have a native HubSpot integration yet. This guide shows you how to connect SalamRuby with HubSpot using automation platforms (Make.com, n8n) or custom webhooks.
</Note>
...
1. **A SalamRuby account** with at least one survey that collects email addresses
...
<Steps>
  ...
  <Step title="Create the App">
    Click **Create a private app** and give it a name (e.g., "SalamRuby Integration").
  </Step>
...
<Steps>
  ...
  <Step title="Create a New Make.com Scenario">
    Go to [Make.com](https://make.com) and create a new scenario. Search for **SalamRuby** and select it as your trigger, then choose **Response Finished** as the trigger event.

    ![Search SalamRuby](/images/xm-and-surveys/core-features/integrations/make/search-salamruby.webp)
  </Step>
  <Step title="Connect SalamRuby to Make">
    Click **Create a webhook**, enter your SalamRuby API Host (default: `https://app.salamruby.com`), add your SalamRuby API Key (see [API Key Setup](/api-reference/rest-api#how-to-generate-an-api-key)), and select the survey you want to connect.
  ...
  <Step title="Add the HubSpot Module">
    Click the **+** button after the SalamRuby trigger, search for **HubSpot**, choose **Create or Update a Contact** as the action, and connect your HubSpot account.
  </Step>
  <Step title="Map SalamRuby Fields to HubSpot">
    Map the SalamRuby response fields to HubSpot contact properties:

    | HubSpot Field | SalamRuby Field |
    | ------------- | ---------------- |
  ...
  <Step title="Test and Activate">
    Submit a test response to your SalamRuby survey, verify the contact appears in HubSpot, and turn on your Make scenario.
  </Step>
</Steps>
...
<Note>
  The SalamRuby n8n node is available as a community node. Install it via **Settings** → **Community Nodes** → install `@salamruby/n8n-nodes-salamruby`.
</Note>

<Steps>
  <Step title="Set Up the SalamRuby Trigger">
    Create a new workflow in n8n, add the **SalamRuby** trigger node, connect it with your SalamRuby API Key and host, select **Response Finished** as the event, and choose your survey.

    ![Add SalamRuby Trigger](/images/xm-and-surveys/core-features/integrations/n8n/add-salamruby-trigger.webp)
  </Step>
  ...
  <Step title="Test Your Workflow">
    Click **Listen for event** in the SalamRuby trigger, submit a test survey response, verify the data flows through to HubSpot, and activate your workflow.
  </Step>
</Steps>
...
For maximum flexibility, you can use SalamRuby webhooks with a custom endpoint that calls the HubSpot API directly. This approach is ideal for developers who want full control.
...
<Steps>
  <Step title="Create a SalamRuby Webhook">
    Go to **Configuration** → **Integrations** in SalamRuby, click **Manage Webhooks** → **Add Webhook**, enter your endpoint URL, select **Response Finished** as the trigger, and choose the surveys to monitor.
  ...
  <Step title="Build Your Webhook Handler">
    ...
    - **Receive the SalamRuby webhook** - Accept POST requests with the survey response payload
  ...
  <Step title="Deploy and Test">
    Deploy your webhook handler to a publicly accessible URL, add the URL to your SalamRuby webhook configuration, submit a test survey response, and verify the contact appears in HubSpot.
  </Step>
</Steps>
...
Still struggling or something not working as expected? [Join our GitHub Discussions](https://github.com/salamruby/salamruby/discussions) and we're happy to help!
