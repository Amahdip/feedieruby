---
title: "Webhooks"
description: "With webhooks, you can receive real-time HTTP notifications when specific objects change in your SalamRuby environment. This allows you to stay updated and trigger automated actions based on these events."
---
...
You can create webhooks either through the **SalamRuby App UI** or programmatically via the **Webhook API**.
...
Use our documented methods on the **Creation**, **List**, and **Deletion** endpoints of the Webhook API mentioned in our [API v2 playground](https://salamruby.com/docs/api-v2-reference/management-api-%3E-webhooks/get-webhooks).
...
<Steps>
  ...
  <Step title="Use the public URL in SalamRuby">
...
By default, SalamRuby blocks webhook URLs that point to private or internal IP addresses (e.g. `localhost`, `192.168.x.x`, `10.x.x.x`) to prevent [SSRF attacks](https://owasp.org/www-community/attacks/Server-Side_Request_Forgery). If you are self-hosting SalamRuby and need to send webhooks to internal services, you can set the following environment variable:
...
If you encounter any issues or need help setting up webhooks, feel free to reach out to us on [GitHub Discussions](https://github.com/salamruby/salamruby/discussions). 😃

---

## Webhook Security with Standard Webhooks

SalamRuby implements the [Standard Webhooks](https://github.com/standard-webhooks/standard-webhooks) specification to ensure webhook requests can be verified as genuinely originating from SalamRuby.
...
When you create a webhook (via the UI or API), SalamRuby automatically generates a unique signing secret for that webhook. The secret follows the Standard Webhooks format: `whsec_` followed by a base64-encoded random value.
...
/**
 * Decodes a SalamRuby webhook secret (whsec_...) to raw bytes
 */
...
/**
 * Verifies a SalamRuby webhook request
...
try {
  verifyWebhook(rawBody, req.headers, process.env.SALAMRUBY_WEBHOOK_SECRET);
...
```json
{
  "data": {
    ...
    "meta": {
      "country": "DE",
      "url": "https://app.salamruby.com/s/surveyId",
      "userAgent": {
        "browser": "Chrome",
        "device": "desktop",
        "os": "macOS"
