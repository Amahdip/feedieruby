...
![SalamRuby Integrations Tab](/images/xm-and-surveys/core-features/integrations/slack/integrations-tab.webp)

1. Now click on the "Connect with Slack" button to authenticate yourself with Slack.

![Connect SalamRuby with your Slack Workspace](/images/xm-and-surveys/core-features/integrations/slack/connect-with-slack.webp)

1. You will now be taken to the Slack OAuth page where you can select the Slack channel you want to link with SalamRuby and click on the "Allow" button.

![Slack OAuth Page](/images/xm-and-surveys/core-features/integrations/slack/slack-auth.webp)

1. Once you have selected the account and completed the authentication process, you will be taken back to SalamRuby Cloud and see the connected status as below:

![SalamRuby is now connected with Slack](/images/xm-and-surveys/core-features/integrations/slack/slack-connected.webp)

<Note>
  Before the next step, make sure that you have a SalamRuby Survey with at
  least one question and a Slack channel in the Slack workspace you integrated.
</Note>

1. In order to make your channel available in channel dropdown, you need to add salamruby integration bot to the channel you want to link. You can do this by going to channel settings -> Integrations -> Add apps -> Search for "SalamRuby" -> Select the bot -> Add.
...
![Add SalamRuby Bot](/images/xm-and-surveys/core-features/integrations/slack/add-slack-bot-4.webp)

1. Now click on the "Link channel" button to link a Slack channel with SalamRuby and a modal will open up.

![Link SalamRuby with a Slack Channel](/images/xm-and-surveys/core-features/integrations/slack/link-survey-with-channel.webp)

1. Select the channel you want to link with SalamRuby and the Survey. On doing so, you will be asked to select the questions' responses you want to feed in the Slack channel. Select the questions and click on the "Link Channel" button.
...
Congratulations! You have successfully linked a Slack channel with SalamRuby. Now whenever a response is submitted for the linked survey, it will be automatically sent to the linked Slack channel.

## Setup in self-hosted SalamRuby

Enabling the Slack Integration in a self-hosted environment requires a setup using slack workspace account and changing the environment variables of your SalamRuby instance.

<Note>
  If you are running SalamRuby locally:
...
5. Add the **Redirect URLs** under **OAuth & Permissions** tab. You can add the following URLs:
   ...
   * If you are running salamruby locally, you can enter `https://localhost:3000/api/v1/integrations/slack/callback`.
...
7. Go to the **Basic Information** tab on the sidebar and copy the **Client ID** and **Client Secret**. Copy them and set them as the environment variables in your SalamRuby instance as:
...
Voila! You have successfully enabled the Slack integration in your self-hosted SalamRuby instance. Now you can follow the steps mentioned in the [SalamRuby Cloud](#salamruby-cloud) section to link a Slack workspace with SalamRuby.

## Remove Integration with Slack Workspace

To remove the integration with Slack Workspace,

1. Visit the Integrations tab in your SalamRuby Cloud dashboard.
...
![Delete Slack Integration with SalamRuby](/images/xm-and-surveys/core-features/integrations/slack/delete-connection.webp)

Still struggling or something not working as expected? [Join our Github Discussions](https://github.com/salamruby/salamruby/discussions) and we'd be glad to assist you!