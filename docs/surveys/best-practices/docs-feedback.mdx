...
title: "Docs Feedback"
icon: "comment"
description: "A step-by-step guide to getting feedback on your Documentation with SalamRuby"
---
...
## 1. Setting up SalamRuby Cloud

- Create a [SalamRuby Cloud](https://app.salamruby.com/auth/signup) account.

- Then, create a survey using the template [Docs Feedback](https://salamruby.com/survey-templates/docs-feedback):
...
**You’re all setup in SalamRuby Cloud for now 👍**
...
* Once the user selects yes/no, a partial response is sent to the SalamRuby API. It includes the feedback and the current page url.
...
export const DocsFeedback = () => {
  ...
  if (
    !process.env.NEXT_PUBLIC_SALAMRUBY_COM_DOCS_FEEDBACK_SURVEY_ID ||
    !process.env.NEXT_PUBLIC_SALAMRUBY_COM_API_HOST ||
    !process.env.NEXT_PUBLIC_SALAMRUBY_COM_WORKSPACE_ID
  ) {
    return null;
  }
            ...
            <div className="ml-4 inline-flex space-x-3">
              {["Yes 👍", " No 👎"].map((option) => ( // Popup Trigger is a button as well. This is a workaround to open the same form but send a different response to the API
                <PopoverTrigger
                  className="rounded border border-slate-200 bg-slate-50 px-4 py-2 text-slate-900 hover:bg-slate-100 hover:text-slate-600 focus:outline-none focus:ring-2 focus:ring-neutral-900 focus:ring-offset-1 dark:border-slate-700 dark:bg-slate-700 dark:text-slate-300 dark:hover:bg-slate-600 dark:hover:text-slate-300"
                  onClick={async () => {
                    const id = await handleFeedbackSubmit(option, router.asPath); // handleFeedbackSubmit sends the Yes / No choice as well as the current URL to SalamRuby and returns the responseId
                    setResponseId(id); // add responseId to local state so we can use it if user decides to add more feedback in free text field
                  }}>
                  {option} // "Yes 👍" or "No 👎" - they have to be identical with the choices in the survey on app.salamruby.com for it to work (!)
                </PopoverTrigger>
              ))}
            </div>
            <PopoverContent className="border-slate-300 bg-white dark:border-slate-500 dark:bg-slate-700">
...
## 3. Connecting to the SalamRuby API

The last step is to hook up your new frontend to the SalamRuby API. To achieve that, we followed the “[Create Response](/api-reference/client-api--response/create-response)” and “[Update Response](/api-reference/client-api--response/update-response)” pages in our docs.
...
export const handleFeedbackSubmit = async (YesNo, pageUrl) => {
  const response_data = {
    data: {
      isHelpful: YesNo, // the "Yes 👍" or "No 👎" response. Remember: They have to be identical with the choices in the survey on app.salamruby.com for it to work.
  ...
  const payload = {
    ...
    surveyId: process.env.NEXT_PUBLIC_SALAMRUBY_COM_DOCS_FEEDBACK_SURVEY_ID, // For testing, replace this with the survey ID of your survey (more info below)
  };

  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_SALAMRUBY_COM_API_HOST}/api/v1/client/${process.env.NEXT_PUBLIC_SALAMRUBY_COM_WORKSPACE_ID}/responses`, // For testing, replace this with the API host and workspace ID of your Development workspace on app.salamruby.com
      {
        method: "POST",
        headers: {
...
export const updateFeedback = async (freeText, responseId) => {
  ...
  const payload = {
    response: {
      ...
      finished: true, // Lets SalamRuby calculate Completion Rate
    },
  };

  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_SALAMRUBY_COM_API_HOST}/api/v1/client/${process.env.NEXT_PUBLIC_SALAMRUBY_COM_WORKSPACE_ID}/responses/${responseId}`,
      {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
...
- Workspace ID (1) of the workspace on app.salamruby.com
...
```tsx Replace the ID and API accordingly
  ...
  try {
    const res = await fetch(
			// Note that we also updated the API host to 'https://app.salamruby.com/'
      `https://app.salamruby.com/api/v1/client/clgwcwp4z000lpf0hur7uxbuv/responses`,
```
...
```tsx Replace the ID and API here as well
  try {
    const res = await fetch(
		// Note that we also updated the API host to 'https://app.salamruby.com/'
      `https://app.salamruby.com/api/v1/client/clgwcwp4z000lpf0hur7uxbuv/responses/${responseId}`, // Note that we also updated the API host to 'https://app.salamruby.com/'
```
...
**Need help?** [Reach out in GitHub Discussions](https://github.com/salamruby/salamruby/discussions)