...
description: "How to Set Up a Product-Market Fit Survey Using SalamRuby - Step-by-Step Guide"
icon: "chart-line-up"
---
...
1. Create new Product-Market Fit survey at [app.salamruby.com](https://app.salamruby.com/)
2. Setup pre-segmentation to assure high data quality
3. Setup the user action to display survey at good point in time

<Note>
  SalamRuby Widget running? You need to have the SalamRuby Widget installed to display the Feedback Box in your app. Please follow [this tutorial (Step 4 onwards)](/xm-and-surveys/surveys/website-app-surveys/quickstart) to install the widget.
</Note>
...
**Filter by attribute**: You can keep the logic to decide if a user has (or has not) experienced value in your application. This makes most sense if you want to use historic usage data to decide if a user qualifies or not. Create your logic and if it applies, send an attribute to SalamRuby by e.g. `salamruby.setAttribute("Loyalty", "Experienced Value");` Here is the full manual on how to [set attributes](/xm-and-surveys/surveys/website-app-surveys/user-identification).
