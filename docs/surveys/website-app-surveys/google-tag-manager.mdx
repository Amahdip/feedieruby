...
description: "Deploy SalamRuby surveys through GTM without modifying your website code."
icon: "tags"
---
...
- Your SalamRuby **Workspace ID** (Settings > Connect Your App)
- Your **App URL**: `https://app.salamruby.com` (or your self-hosted URL)
...
<Steps>
  <Step title="Create a Custom HTML tag in GTM">
    1. Create a new tag with preferred name e.g. "SalamRuby Intercept Surveys"
...
!function(){
    var appUrl = "https://app.salamruby.com"; // REPLACE ONLY IF YOUR SELF-HOST
    ...
    t.onload=function(){
        window.salamruby && window.salamruby.setup({
...
![Add GTM Custom HTML tag](/images/xm-and-surveys/surveys/website-app-surveys/google-tag-manager/create-a-tag.webp)
  ...
  <Step title="Test">
    ...
    3. Add `?salamrubyDebug=true` to the URL to see test logs in browser console (see [Debugging Mode](/xm-and-surveys/surveys/website-app-surveys/framework-guides#debugging-salamruby-integration) for more details)
  </Step>
</Steps>
...
<Note>
  User identification is part of the SalamRuby [Enterprise Edition](/self-hosting/advanced/license). 
</Note>
...
<Steps>
  ...
  <Step title="Create identification tag">
    New Custom HTML tag named "SalamRuby - User":

```html
<script>
(function() {
    var check = setInterval(function() {
        if (window.salamruby && window.salamruby.setUserId) {
            clearInterval(check);
            var userId = {{User ID}};
            if (userId) {
                window.salamruby.setUserId(userId);
                var attrs = {};
                if ({{User Email}}) attrs.email = {{User Email}};
                if ({{User Plan}}) attrs.plan = {{User Plan}};
                if (Object.keys(attrs).length) {
                    window.salamruby.setAttributes(attrs);
                }
            }
        }
    }, 100);
...
```
  ...
  <Step title="Set trigger and push data">
    ...
    4. Attach this trigger to your "SalamRuby - User" tag
    5. Save and publish
...
<Steps>
  <Step title="Create code action in SalamRuby">
    Add code action via SalamRuby UI
...
```html
<script>
if (window.salamruby && window.salamruby.track) {
    window.salamruby.track({{Event Name}});
}
</script>
```
...
- Add `?salamrubyDebug=true` to your URL
...
**Events not tracking?**
- Confirm `window.salamruby` exists before calling track
- Match event names exactly with SalamRuby action names
- Check timing - SalamRuby must be initialized first

## Need Help?

- [GitHub Discussions](https://github.com/salamruby/salamruby/discussions)
