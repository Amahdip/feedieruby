---
title: "Actions"
description: "Actions are predefined events within your app that prompt SalamRuby to display a survey when triggered. These are detected by the SalamRuby widget, which then presents the appropriate survey based on your predefined settings."
icon: "code"
---
...
<Step title="SalamRuby widget detects action">
...
<Step title="Check for survey trigger">
  If a survey is set to trigger on that action, SalamRuby checks if the user or visitor qualifies.
</Step>
  ...
  <Step title="Check for user or visitor qualification">
    ...
    Also, if this user or visitor has seen this specific survey before, SalamRuby might not show it as this is dependent on the [Recontact Options](/xm-and-surveys/surveys/website-app-surveys/recontact).
...
SalamRuby offers an intuitive No-Code interface that allows you to configure actions without needing to write any code.
...
<Steps>
  <Step title="Visit the Settings tab and click on `User Actions`">
    ![Action overview on SalamRuby Open Source Survey Solution](/images/xm-and-surveys/surveys/website-app-surveys/actions/actions-view.webp "Action overview on SalamRuby Open Source Survey Solution")
  </Step>
...
<Steps>
  <Step title="Configure action in SalamRuby">
    First, add the action via the SalamRuby web interface to make it available for survey configuration:
...
![Add a code action to open source in app survey](/images/xm-and-surveys/surveys/website-app-surveys/actions/code-action.webp "Add a code action to open source in app survey")
  ...
  <Step title="Add action tracking to your code">
    Use salamruby.track() to send an action event to SalamRuby:

    ```
    salamruby.track("action");
    ```
    ...
    ```javascript
    const handleClick = () => {
      salamruby.track("Button Clicked");
    };
