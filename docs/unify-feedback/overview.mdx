...
Unify Feedback is the part of SalamRuby that consolidates feedback from across your stack into a single, queryable store. Survey responses, CSV imports, API ingestions, and tool-generated records all land in the same model so you can analyze them together.
...
2. **Connect Sources.** Pull data from SalamRuby surveys, upload CSVs, or push records via the API.
...
<Note>
  Unify Feedback is an enterprise feature. Enable it on SalamRuby Cloud with a paid plan, or self-host with a license.
</Note>
