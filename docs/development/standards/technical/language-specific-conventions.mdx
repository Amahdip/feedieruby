...
description: This document outlines the language-specific conventions for the SalamRuby codebase, providing guidelines for writing code in TypeScript/JavaScript.
icon: code
---
...
2. **Architecture Patterns**
   ...
   - **Singleton Pattern**: Main SDK class (`SalamRuby`) with static interface
   - **Manager Pattern**: Specialized managers (`UserManager`, `SurveyManager`, `PresentSurveyManager`)
   - **Builder Pattern**: Configuration objects (`SalamRubyConfig.Builder`)
   - **Protocol-Oriented Programming**: Service protocols for dependency injection and testing

3. **Error Handling**

   - Custom error enums with descriptive cases (`SalamRubySDKErrorType`)
...
4. **Naming Conventions**
   ...
   - Classes: PascalCase (`SalamRubyConfig`, `UserManager`)
   - Properties/Methods: camelCase (`workspaceId`, `setUserId`)
   - Constants: camelCase with descriptive names
   - Protocol names: Descriptive with "Protocol" suffix (`SalamRubyServiceProtocol`)
...
```swift
// Class definitions
@objc(SalamRuby) public class SalamRuby: NSObject {
...
@objc public static func setup(with config: SalamRubyConfig, force: Bool = false)

// Guard statements for early returns
guard !isInitialized else {
    let error = SalamRubySDKError(type: .sdkIsAlreadyInitialized)
    SalamRuby.logger?.error(error.message)
    return
}

// Enum cases with descriptive names
public enum SalamRubySDKErrorType: Int {
...
2. **Kotlin Language Features**
   ...
   - **Object singletons** for stateless utilities and managers (`SalamRuby`, `Logger`, `SDKError`)
