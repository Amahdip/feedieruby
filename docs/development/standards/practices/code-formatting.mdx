...
description: "Standards for consistent code formatting across the SalamRuby codebase"
icon: code
---
...
### Prettier Config
SalamRuby uses a standardized Prettier configuration defined in `@salamruby/config-prettier`:
```javascript
module.exports = {
  bracketSpacing: true,
...
- Consistent use of `tsconfig.json` extending from `@salamruby/config-typescript`
...
```json
{
  ...
  "extends": "@salamruby/config-typescript/js-library.json",
...
Each package extends from `@salamruby/eslint-config` with specific presets:
...
These standards ensure consistency across the SalamRuby codebase while maintaining high code quality and developer productivity.
