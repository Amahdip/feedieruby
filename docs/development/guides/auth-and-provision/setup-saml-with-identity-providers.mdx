...
description: "This guide explains the settings you need to use to configure SAML with your Identity Provider. Once configured, obtain an XML metadata file and use it to configure SAML in SalamRuby."
---

### SAML Registration with Identity Providers

This guide explains the settings you need to use to configure SAML with your Identity Provider. Once configured, obtain an XML metadata file and use it to configure SAML in SalamRuby.

> **Note:** Please do not add a trailing slash at the end of the URLs. Create them exactly as shown below.

**Assertion consumer service URL / Single Sign-On URL / Destination URL:** [https://app.salamruby.com/api/auth/saml/callback](https://app.salamruby.com/api/auth/saml/callback)

**Entity ID / Identifier / Audience URI / Audience Restriction:** [https://saml.salamruby.com](https://saml.salamruby.com)

> **Note:** [https://saml.salamruby.com](https://saml.salamruby.com) is hardcoded in SalamRuby — do not replace it with your instance URL. It is the fixed SP Entity ID and must match exactly as shown in SAML assertions.
...
<Steps>
  ...
  <Step title="Enter the SAML Integration Settings as shown and click Next">
    - **Single Sign-On URL**: `https://<your-salamruby-instance>/api/auth/saml/callback` or `http://localhost:3000/api/auth/saml/callback` (if you are running SalamRuby locally)
    - **Audience URI (SP Entity ID)**: `https://saml.salamruby.com` (hardcoded; do not replace with your instance URL)
  ...
  <Step title="Copy the IDP metadata and paste it in the `connection.xml` file in the `salamruby/saml-connection` (use `salamruby/apps/web/saml-connection` for development) directory" />
</Steps>
