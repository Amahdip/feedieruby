...
This usually happens when the SalamRuby Widget isn't correctly or completely built.

```bash
pnpm build --filter=@salamruby/js
...
```bash helloWorld.js
pnpm dev --filter=@salamruby/web...
```

It’s better to use a single terminal with `pnpm dev` rather than having multiple open (one with the SalamRuby app and one with the demo).
