---
title: "MCP Server"
description: "Configure and use the SalamRuby v3 Surveys MCP server"
icon: "robot"
---

## Overview

The SalamRuby MCP server exposes a small v3 Surveys tool surface for AI agents. It runs inside the
SalamRuby web app at `/api/mcp` and reuses the same v3 Surveys API authentication, authorization,
...
Use the streamable HTTP MCP endpoint on the SalamRuby app:

```text
https://app.salamruby.com/api/mcp
```
...
response headers. Browser-origin MCP requests must come from the configured public SalamRuby origin.

## Authentication

Authenticate with a SalamRuby API key in a request header:
...
Start SalamRuby and prepare the database:

```bash
pnpm db:up
pnpm db:migrate:dev
pnpm --filter @salamruby/web dev
```

Create an API key in the SalamRuby app with access to the target workspace. Use the least privileged
permission needed by the agent workflow.

Verify that the MCP endpoint can list tools:

```bash
export SALAMRUBY_MCP_API_KEY="fbk_..."

curl -sS -X POST http://localhost:3000/api/mcp \
  -H "Accept: application/json, text/event-stream" \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer $SALAMRUBY_MCP_API_KEY" \
...
```bash
launchctl setenv SALAMRUBY_MCP_API_KEY "fbk_..."
```

Register the local MCP server:

```bash
codex mcp add salamruby-local \
  --url http://localhost:3000/api/mcp \
  --bearer-token-env-var SALAMRUBY_MCP_API_KEY
```

Equivalent Codex config:

```toml
[mcp_servers.salamruby-local]
url = "http://localhost:3000/api/mcp"
bearer_token_env_var = "SALAMRUBY_MCP_API_KEY"
```
...
```bash
codex mcp list
codex mcp get salamruby-local
```

Then ask Codex to use the configured server:

```text
Use the salamruby-local MCP server to list surveys for workspace clxx1234567890123456789012.
```
...
```bash
claude mcp add --transport http salamruby-local http://localhost:3000/api/mcp \
  --header "Authorization: Bearer $SALAMRUBY_MCP_API_KEY"
```
...
```bash
claude mcp list
claude mcp get salamruby-local
```
...
```json
{
  "mcpServers": {
    "salamruby-local": {
      "headers": {
        "Authorization": "Bearer ${SALAMRUBY_MCP_API_KEY}"
      },
      "type": "http",
      "url": "http://localhost:3000/api/mcp"
    }
...
Claude Code will fail to parse this config if `SALAMRUBY_MCP_API_KEY` is not set.
