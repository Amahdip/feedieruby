---
title: "Docker Setup"
description: "Set up SalamRuby quickly using our ready-to-use Docker image."
icon: "docker"
---
...
<Info>
  Starting with SalamRuby v5, the production Docker Compose stack includes SalamRuby Hub and Cube as part of
...
1. **Create a New Directory for SalamRuby**

   Open a terminal and run the following commands to create and enter a new directory for SalamRuby:

   ```bash
   mkdir salamruby-quickstart && cd salamruby-quickstart
   ```
...
1. **Download the Docker Files**
   ...
   ```bash
   mkdir -p cube/schema
   curl -o docker-compose.yml https://raw.githubusercontent.com/salamruby/salamruby/stable/docker/docker-compose.yml
   curl -o cube/cube.js https://raw.githubusercontent.com/salamruby/salamruby/stable/docker/cube/cube.js
   curl -o cube/schema/FeedbackRecords.js https://raw.githubusercontent.com/salamruby/salamruby/stable/docker/cube/schema/FeedbackRecords.js
   ```

1. **Generate Hub and Cube Secrets**

   SalamRuby Hub and Cube each require a shared secret. Create `.env` with both:
...
1. **Generate Hub API Key**
   ...
   SalamRuby v5 requires a Hub API key for the bundled Hub service.
   ...
   <Info>
     ...
     change that value if your SalamRuby app needs to reach Hub at a different address. If your deployment
...
1. **Start the Docker Setup**
   ...
   Now, you're ready to run SalamRuby with Docker. Use the command below to start SalamRuby together with
   PostgreSQL, Redis, SalamRuby Hub, and Cube.
...
1. **Open SalamRuby in Your Browser**

   Once the setup is running, open [**http://localhost:3000**](http://localhost:3000) in your browser to access SalamRuby. The first time you visit, you'll see a setup wizard. Follow the steps to create your first user and start using SalamRuby.

<Note>
  The bundled Docker stack keeps SalamRuby Hub and Cube internal to the compose network. The app reaches them
  through `http://hub:8080` and `http://cube:4000`.
</Note>

<Info>
  If you use the one-click Traefik setup, FeedbackRecords are available on the SalamRuby origin at
  `/api/v3/feedbackRecords` and `/v1/feedback-records`. Custom Docker reverse proxies need equivalent wiring:
  run gateway auth against the SalamRuby app, rewrite `/api/v3/feedbackRecords` to Hub's
  `/v1/feedback-records`, and inject `Authorization: Bearer <HUB_API_KEY>` only on the Hub-bound hop.
</Info>

## Update

Please take a look at our [migration guide](/self-hosting/advanced/migration) for version specific steps to update SalamRuby.

<Info>
  For a major migration such as SalamRuby 4.x to 5.0, update your compose structure and configuration first.
...
1. Pull the latest SalamRuby image

   ```bash
   docker compose pull
   ```

1. Stop the SalamRuby stack

   ```bash
   docker compose down
   ```

1. Re-start the SalamRuby stack with the updated image
...
RustFS provides S3-compatible object storage for file uploads in SalamRuby. If you want to enable features
like image uploads, survey file uploads, or custom logos, you can run RustFS alongside SalamRuby while
...
starts RustFS and auto-creates the `salamruby` bucket.
...
1. **Start the local stack**
   ...
   bootstrap job, SalamRuby Hub, and a local Cube instance for analytics testing.
...
4. **Verify uploads**
   ...
   After uploading a file in SalamRuby, open http://localhost:9001 and navigate to **Buckets → salamruby**
...
services:
  ...
  rustfs:
    ...
    environment:
      ...
      # CORS for browser uploads: set to your SalamRuby origin(s), comma-separated.
...
```bash
SALAMRUBY_RUSTFS_ADMIN_USER=salamruby-root
SALAMRUBY_RUSTFS_ADMIN_PASSWORD=change-this-secure-password
SALAMRUBY_RUSTFS_SERVICE_USER=salamruby-service
SALAMRUBY_RUSTFS_SERVICE_PASSWORD=change-this-service-password
SALAMRUBY_RUSTFS_BUCKET_NAME=salamruby
SALAMRUBY_RUSTFS_POLICY_NAME=salamruby-policy
SALAMRUBY_RUSTFS_REGION=us-east-1
```

Then configure SalamRuby to use the RustFS service credentials:

```bash
S3_ACCESS_KEY="${SALAMRUBY_RUSTFS_SERVICE_USER}"
S3_SECRET_KEY="${SALAMRUBY_RUSTFS_SERVICE_PASSWORD}"
S3_REGION="${SALAMRUBY_RUSTFS_REGION}"
S3_BUCKET_NAME="${SALAMRUBY_RUSTFS_BUCKET_NAME}"
...
- **Health check**: From the SalamRuby container:

  ```bash
  docker compose exec salamruby sh -c 'wget -O- http://rustfs:9000/health'
  ```
...
- CORS configuration scoped to your SalamRuby domain
- Rate limiting middleware
- Separate RustFS admin and SalamRuby service credentials
- A `rustfs-init` job that creates the bucket and access policy

The production setup from [salamruby.sh](https://github.com/salamruby/salamruby/blob/main/docker/salamruby.sh)
...
<Note>
  ...
  storage or a dedicated RustFS deployment managed separately from SalamRuby.
</Note>
...
```bash
[+] Running 9/16
⠹ salamruby 15 layers [⣿⣤⣿⣿⣿⣿⣿⣿⣿⣿⠀⠀⠀⠀⠀] 29.78MB/47.76MB Pulling                                                           13.3s
...
[+] Running 2/2
✔ Container salamruby-quickstart-postgres-1    Created                                                                       0.0s
✔ Container salamruby-quickstart-salamruby-1  Created                                                                       0.0s
```

And at the tail of the output, you should see this:

```bash
salamruby-quickstart-salamruby-1  | All migrations have been successfully applied.
salamruby-quickstart-salamruby-1  |
salamruby-quickstart-salamruby-1  | - info Loaded env from /home/nextjs/apps/web/.env
salamruby-quickstart-salamruby-1  | Listening on port 3000 url: http://<random-string>:3000
```
