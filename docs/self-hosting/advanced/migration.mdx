---
title: "Migration"
description: "SalamRuby Self-hosted version migration"
icon: "arrow-right"
---

## v5

SalamRuby v5 changes the self-hosted runtime contract. If you are upgrading an existing SalamRuby 4.x
deployment, review this section before starting the new version.

### What Changes In v5

- **SalamRuby Hub is now mandatory** for self-hosted SalamRuby v5 deployments.
...
<Warning>
  SalamRuby v5 removes application-level rate limiting for several routes that are now expected to be
...
Before you restart your instance on SalamRuby v5:
...
SalamRuby v5 expects Hub to be part of the self-hosted stack.

- `HUB_API_KEY` is required
- `HUB_API_URL` must point to the Hub service the SalamRuby app can reach
- `HUB_DATABASE_URL` is optional; when unset, Hub can share the same PostgreSQL database as SalamRuby
...
<Note>
  ...
  [SalamRuby Hub repository](https://github.com/salamruby/hub). For a normal SalamRuby v5 upgrade, use that
  repository as reference material only; the canonical migration steps stay in these SalamRuby docs.
</Note>

#### Edge Rate Limiting

SalamRuby v5 splits rate limiting across two layers:
...
Cube is part of the baseline SalamRuby v5 stack.

- the Docker, one-click, and Helm deployments all bundle the `cube` service by default
- the SalamRuby app requires `CUBEJS_API_URL` and `CUBEJS_API_SECRET`; the install/dev-setup scripts
...
<Tabs>
  <Tab title="One-Click">
    From the `salamruby` directory created by the installer:

    ```bash
    cd salamruby
    docker compose exec postgres pg_dump -Fc -U postgres -d salamruby > salamruby_pre_v5_$(date +%Y%m%d_%H%M%S).dump
    ```
  ...
  <Tab title="Kubernetes">
    ...
    ```bash
    kubectl exec -n salamruby salamruby-postgresql-0 -- pg_dump -Fc -U salamruby -d salamruby > salamruby_pre_v5_$(date +%Y%m%d_%H%M%S).dump
    ```
...
<Tabs>
  <Tab title="One-Click">
    The one-click `./salamruby.sh update` command only pulls new images. It does **not** rewrite your existing
    `salamruby/docker-compose.yml`, so you must merge the v5 stack changes before the first v5 restart.

    ```bash
    curl -fsSL -o salamruby/docker-compose.v5.yml https://raw.githubusercontent.com/salamruby/salamruby/stable/docker/docker-compose.yml
    ```

    Then compare `salamruby/docker-compose.v5.yml` with your existing `salamruby/docker-compose.yml` and merge
    the v5 additions:
    ...
    - include the bundled `salamruby-migrate`, `hub-migrate`, and `hub` services
    - sync `salamruby/cube/cube.js` and `salamruby/cube/schema/FeedbackRecords.js` from the current
      release and ensure `salamruby/.env` contains `CUBEJS_API_SECRET` (Cube is part of the baseline stack
      in v5)
  ...
  <Tab title="Docker Compose">
    ...
    ```bash
    curl -fsSL -o docker-compose.v5.yml https://raw.githubusercontent.com/salamruby/salamruby/stable/docker/docker-compose.yml
    ```
    ...
    - the compose stack includes `salamruby-migrate`, `hub-migrate`, and `hub`
  ...
  <Tab title="Kubernetes">
    Upgrade using the current SalamRuby chart:
...
- confirm the SalamRuby app starts and `GET /health` returns successfully
- confirm the Hub service is healthy and reachable from the SalamRuby app
...
- **Missing `HUB_API_KEY`**: SalamRuby cannot authenticate to Hub, and Hub itself may fail to start
...
- **Missing `CUBEJS_API_SECRET`** (or unreachable Cube endpoint): the SalamRuby app fails env validation
...
- revert your image tags or Helm release values to the last known-good SalamRuby 4.x version
...
<Info>
  SalamRuby v5 also includes the Environment ID to Workspace ID transition for SDK setup. If you still rely on a
  legacy Environment ID, read the [Migrate to Workspace ID guide](/surveys/website-app-surveys/workspace-id-migration).
</Info>

## v4.7

SalamRuby v4.7 introduces **typed contact attributes** with native `number` and `date` data types. This enables comparison-based segment filters (e.g. "signup date before 2025-01-01") that were previously not possible with string-only attribute values.

### What Happens Automatically

When SalamRuby v4.7 starts for the first time, the data migration will:
...
<Tabs>
  <Tab title="Docker">
    Before running these steps, navigate to the `salamruby` directory where your `docker-compose.yml` file is located.

    ```bash
    docker exec salamruby-postgres-1 pg_dump -Fc -U postgres -d salamruby > salamruby_pre_v4.7_$(date +%Y%m%d_%H%M%S).dump
    ```

    <Info>
      If you run into "**No such container**", use `docker ps` to find your container name, e.g.
      `salamruby_postgres_1`.
    </Info>
  ...
  <Tab title="Kubernetes">
    ...
    ```bash
    kubectl exec -n salamruby salamruby-postgresql-0 -- pg_dump -Fc -U salamruby -d salamruby > salamruby_pre_v4.7_$(date +%Y%m%d_%H%M%S).dump
    ```

    <Info>
      If your PostgreSQL pod has a different name, run `kubectl get pods -n salamruby` to find it.
    </Info>
...
After SalamRuby starts, check the logs to see whether the value backfill was completed or skipped:

<Tabs>
  <Tab title="Docker">
    ```bash
    docker compose logs salamruby | grep -i "backfill"
    ```
  </Tab>
  <Tab title="Kubernetes">
    ```bash
    # Check the application pod logs
    kubectl logs -n salamruby -l app.kubernetes.io/name=salamruby --tail=200 | grep -i "backfill"
    ```

    If the Helm migration Job ran, you can also inspect its logs:

    ```bash
    kubectl logs -n salamruby job/salamruby-migration
    ```
...
If the migration skipped the value backfill, run the standalone backfill script inside the running SalamRuby container:

<Tabs>
  <Tab title="Docker">
    ```bash
    docker exec salamruby node packages/database/dist/scripts/backfill-attribute-values.js
    ```

    <Info>Replace `salamruby` with your actual container name if it differs. Use `docker ps` to find it.</Info>

  </Tab>
  <Tab title="Kubernetes">
    ```bash
    kubectl exec -n salamruby deploy/salamruby -- node packages/database/dist/scripts/backfill-attribute-values.js
    ```

    <Info>
      If your SalamRuby deployment has a different name, run `kubectl get deploy -n salamruby` to find it.
    </Info>
...
- **Safe to run while SalamRuby is live** -- it does not lock the entire table or wrap work in a long transaction
...
- Access your SalamRuby instance at the same URL as before
...
SalamRuby 4 introduces additional requirements for self-hosting setups and makes a dedicated Redis cache as well as S3-compatible file storage mandatory.

</Warning>

SalamRuby 4.0 is a **major milestone** that sets up the technical foundation for future iterations and feature improvements. This release focuses on modernizing core infrastructure components to improve reliability, scalability, and enable advanced features going forward.

### What's New in SalamRuby 4.0
...
These improvements in SalamRuby 4.0 also make some infrastructure requirements mandatory going forward:
...
Additional migration steps are needed if you are using a self-hosted SalamRuby setup that uses either local file storage (not S3-compatible file storage) or doesn't already use a Redis cache.
...
curl -fsSL -o migrate-to-v4.sh \
  https://raw.githubusercontent.com/salamruby/salamruby/stable/docker/migrate-to-v4.sh
...
- Pulls the latest SalamRuby image and updates your instance

### Manual Setup

If you use a different setup to host your SalamRuby instance, you need to make sure to make the necessary adjustments to run SalamRuby 4.0.

#### Redis

SalamRuby 4.0 requires a Redis instance to work properly. Please add a Redis instance to your Docker setup, your K8s infrastructure, or however you are hosting SalamRuby at the moment. SalamRuby works with the latest versions of Redis as well as Valkey.

You need to configure the `REDIS_URL` environment variable and point it to your Redis instance.

#### S3-compatible storage

To use file storage (e.g., file upload questions, image choice questions, custom survey backgrounds, etc.), you need to have S3-compatible file storage set up and connected to SalamRuby.

SalamRuby supports multiple storage providers (among many other S3-compatible storages):
...
Please make sure to set up a storage bucket with one of these solutions and then link it to SalamRuby using the following environment variables:

```
  S3_ACCESS_KEY: your-access-key
  S3_SECRET_KEY: your-secret-key
  S3_REGION: us-east-1
  S3_BUCKET_NAME: salamruby-uploads
...
**Critical Step**: Create a complete database backup before proceeding. SalamRuby 4.0 will automatically remove unused database tables and fields during startup.

```bash
docker exec salamruby-postgres-1 pg_dump -Fc -U postgres -d salamruby > salamruby_pre_v4.0_$(date +%Y%m%d_%H%M%S).dump
```
...
# Start with SalamRuby 4.0
docker compose up -d
```

**3. Automatic Database Migration**

When you start SalamRuby 4.0 for the first time, it will **automatically**:
...
<Info>
  With SalamRuby 3.0, we introduced an automatic data migration system. If you're using a version older than
...
This guide is for users who are **self-hosting SalamRuby** using the **one-click setup**. If you have a different setup, you may need to adjust the commands accordingly.
...
```bash
docker exec salamruby-postgres-1 pg_dump -Fc -U postgres -d salamruby > salamruby_pre_v3.3_$(date +%Y%m%d_%H%M%S).dump
```

<Info>
  If you run into “**No such container**”, use `docker ps` to find your container name,
  e.g. `salamruby_postgres_1`.
</Info>

If you prefer storing the backup as an `*.sql` file remove the `-Fc` (custom format) option. In case of a restore scenario you will need to use `psql` then with an empty `salamruby` database.

1. Pull the latest version of SalamRuby:

```bash
docker compose pull
```

1. Stop the running SalamRuby instance & remove the related containers:

```bash
docker compose down
```

1. Restarting the containers with the latest version of SalamRuby:

```bash
docker compose up -d
```

When you start the latest version of **SalamRuby**, it will **automatically detect and apply any required data migrations** during startup. You don’t need to run any manual migration steps or download separate migration images.

- Access your updated instance

Once the containers are running, simply go to the **same URL** as before to access your updated **SalamRuby** instance.

That’s it! This new process ensures your **SalamRuby** setup stays up to date with the latest schema changes as soon as you run the updated container.
...
```bash
docker exec salamruby-postgres-1 pg_dump -Fc -U postgres -d salamruby > salamruby_pre_v3.2_$(date +%Y%m%d_%H%M%S).dump
```
...
```bash
docker exec salamruby-postgres-1 pg_dump -Fc -U postgres -d salamruby > salamruby_pre_v3.1_$(date +%Y%m%d_%H%M%S).dump
```
...
With **SalamRuby 3.0**, we're making changes to ensure long-term sustainability while still supporting open source. While the **Community Edition** has gained [new features](https://salamruby.com/blog/salamruby-3-0), some [advanced capabilities](https://salamruby.com/docs/self-hosting/license) are now part of the **Enterprise Edition**.

⚠️ **No Downgrade Option:** If you upgrade to **3.0** and run the data migration, **you cannot revert to 2.7.2**. If you rely on **SSO, user identification, or cluster support**, either **stay on version 2.7.x** or reach out to us on [**GitHub Discussions**](https://github.com/salamruby/salamruby/discussions) **for a custom quote**.

</Warning>

This major release brings a better approach to **data migrations**.

- If you're using **SalamRuby v2.7 or later**, you can upgrade **directly** to **v3.x** and future versions.

- If you're on a version **older than v2.7**, you **must upgrade step-by-step** (e.g. **2.4 → 2.5 → 2.6 → 2.7**) before moving to v3.x.

### Steps to Migrate

This guide is for users **self-hosting** SalamRuby with the **one-click setup**. If you're using a different setup, you might adjust the commands.

- &#x20;Navigate to the SalamRuby Directory
...
```bash
docker exec salamruby-postgres-1 pg_dump -Fc -U postgres -d salamruby > salamruby_pre_v3.0_$(date +%Y%m%d_%H%M%S).dump
...
<Info>
  ...
  to restore it later, use `psql` with an empty SalamRuby database.
</Info>

- Pull the latest version of SalamRuby:

```bash
docker compose pull
```

- Stop the running SalamRuby instance & remove the related containers:

```bash
docker compose down
```

- Restarting the containers with the latest version of SalamRuby:

```bash
docker compose up -d
```

When you start the latest version of SalamRuby, it will automatically detect and run any necessary data migrations during startup. There is no need to run any manual migration steps or pull any separate migration images.

- Access your updated instance

That’s it! Once the containers are up and running, simply navigate to the same URL as before to access your fully migrated SalamRuby instance.

## v2.7

<Info>
  AI Features & Database Update: This release lays the groundwork for upcoming AI features, now in private beta. SalamRuby requires the `pgvector` extension in PostgreSQL.

One-Click Setup Users: Use the `pgvector/pgvector:pg15` image instead of `postgres:15-alpine` to ensure compatibility.

</Info>

SalamRuby v2.7 includes all the features and improvements developed by the community during Hacktoberfest 2024. Additionally, this release introduces an advanced team-based access control system, which is available in the SalamRuby Enterprise Edition.
...
This guide is for users who are self-hosting SalamRuby using our one-click setup. If you are using a different setup, you might adjust the commands accordingly.

To run all these steps, please navigate to the SalamRuby folder where your `docker-compose.yml` file is located.
...
```bash
docker exec salamruby-postgres-1 pg_dump -Fc -U postgres -d salamruby > salamruby_pre_v2.7_$(date +%Y%m%d_%H%M%S).dump
```
...
- Stop the running SalamRuby instance and remove the related containers:
...
```bash
docker pull ghcr.io/salamruby/data-migrations:latest && \
docker run --rm \
  --network=salamruby_default \
  -e DATABASE_URL="postgresql://postgres:postgres@postgres:5432/salamruby?schema=public" \
  -e UPGRADE_TO_VERSION="v2.7" \
  ghcr.io/salamruby/data-migrations:v2.7.0
...
- That's it! Once the migration is finished, you can **access your SalamRuby instance** at the same URL as before.

## v2.6

SalamRuby v2.6 introduces advanced logic jumps for surveys, allowing more complex branching logic, including variables, conditions, and more. This release also includes numerous bug fixes, significant performance improvements for website and app surveys, and enhanced stability.
...
This guide is for users who are self-hosting SalamRuby using our one-click setup. If you're using a different setup, you might need to adjust the commands accordingly.
...
```bash
docker exec salamruby-postgres-1 pg_dump -Fc -U postgres -d salamruby > salamruby_pre_v2.6_$(date +%Y%m%d_%H%M%S).dump
```
...
<Info>
  ...
  restore scenario, you will need to use `psql` then with an empty `salamruby` database.
</Info>
...
docker run --rm \
  ...
  ghcr.io/salamruby/data-migrations:v2.6.0
```
...
- Once the migration is complete, **you can access your SalamRuby instance** at the same URL as before.

## v2.5

SalamRuby v2.5 lets you visualise responses in a data table format. This release also brings a few bug fixes and performance improvements.
...
This guide is for users self-hosting SalamRuby with our one-click setup. If you're using a different setup, you may need to adjust the commands.
...
```bash
docker exec salamruby-postgres-1 pg_dump -Fc -U postgres -d salamruby > salamruby_pre_v2.5_$(date +%Y%m%d_%H%M%S).dump
```
...
docker run --rm \
  ...
  ghcr.io/salamruby/data-migrations:v2.5.3
```
...
SalamRuby v2.4 lets you create multiple endings for your surveys and choose which one users see based on logic jumps. This version also includes bug fixes and performance improvements.
...
<Info>
  ...
  triggers, but they will no longer be stored on the server to improve SalamRuby' overall performance.
</Info>

### Steps to Migrate

This guide is for users who are self-hosting SalamRuby with our one-click setup. If you're using a different setup, you may need to adjust the commands accordingly.
...
```bash
docker exec salamruby-postgres-1 pg_dump -Fc -U postgres -d salamruby > salamruby_pre_v2.4_$(date +%Y%m%d_%H%M%S).dump
```
...
```bash
docker pull ghcr.io/salamruby/data-migrations:v2.4.3 && \
docker run --rm \
  --network=salamruby_default \
  -e DATABASE_URL="postgresql://postgres:postgres@postgres:5432/salamruby?schema=public" \
  -e UPGRADE_TO_VERSION="v2.4" \
  ghcr.io/salamruby/data-migrations:v2.4.3
```
...
- That's it! Once the migration is done, you can access your SalamRuby instance at the same URL as before.
...
SalamRuby v2.3 introduces new colour options for rating questions, enhanced multi-language support for Chinese (Simplified & Traditional), and includes several bug fixes and performance improvements.
...
This guide is for users self-hosting SalamRuby with our one-click setup. If you're using a different setup, adjust the commands as needed.
...
```bash
docker exec salamruby-postgres-1 pg_dump -Fc -U postgres -d salamruby > salamruby_pre_v2.3_$(date +%Y%m%d_%H%M%S).dump
```
...
```bash
docker pull ghcr.io/salamruby/data-migrations:v2.3.0 && \
docker run --rm \
  --network=salamruby_default \
  -e DATABASE_URL="postgresql://postgres:postgres@postgres:5432/salamruby?schema=public" \
  -e UPGRADE_TO_VERSION="v2.3" \
  ghcr.io/salamruby/data-migrations:v2.3.0
...
The feature to create short URLs in SalamRuby is now deprecated. Existing short URLs will continue to work for now, but it is recommended to use long URLs instead. Support for short URL redirects will be removed in a future release.

## v2.2

SalamRuby v2.2 introduces XM research presets, providing a brand-new product onboarding experience. Our goal is to make user research "obviously easy" across different industries, starting with Software-as-a-Service and E-Commerce.
...
```bash
docker exec salamruby-postgres-1 pg_dump -Fc -U postgres -d salamruby > salamruby_pre_v2.2_$(date +%Y%m%d_%H%M%S).dump
```
...
```bash
docker pull ghcr.io/salamruby/data-migrations:v2.2 && \
docker run --rm \
  --network=salamruby_default \
  -e DATABASE_URL="postgresql://postgres:postgres@postgres:5432/salamruby?schema=public" \
  -e UPGRADE_TO_VERSION="v2.2" \
  ghcr.io/salamruby/data-migrations:v2.2
```
...
SalamRuby v2.1 introduces more options for creating No-Code Actions and improves the self-hosting experience with a new onboarding process for fresh instances.
...
This guide is for users self-hosting SalamRuby using our one-click setup. If you are using a different setup, you may need to adjust the commands accordingly.

To follow these steps, please navigate to the SalamRuby folder where your `docker-compose.yml` file is located.
...
```bash
docker exec salamruby-postgres-1 pg_dump -Fc -U postgres -d salamruby > salamruby_pre_v2.1_$(date +%Y%m%d_%H%M%S).dump
```
...
```bash
docker pull ghcr.io/salamruby/data-migrations:v2.1.0 && \
docker run --rm \
  --network=salamruby_default \
  -e DATABASE_URL="postgresql://postgres:postgres@postgres:5432/salamruby?schema=public" \
  -e UPGRADE_TO_VERSION="v2.1" \
  ghcr.io/salamruby/data-migrations:v2.1.0
```

Once the migration script runs successfully, your data will be migrated to the latest schema. The script is designed to run safely multiple times, so you don’t need to worry if it’s executed more than once. After completing the migration, you can continue to access your SalamRuby instance at the same URL as before.

- That's it! Once the migration is complete, you can **now access your SalamRuby instance** at the same URL as before.
...
SalamRuby v2.0 introduces significant features such as **Multi-Language Surveys** and **Advanced Styling for Surveys**, aimed at improving the flexibility and appearance of your surveys. Additionally, the release includes several optimisations, bug fixes, and smaller improvements to ensure a smoother user experience.

If you are upgrading to SalamRuby v2.0, follow this guide to ensure a smooth migration of your existing SalamRuby instance without encountering issues or build errors.

<Info>
  This upgrade requires a **data migration**. Please make sure to back up your database before proceeding.
  Follow the steps below to upgrade your SalamRuby instance to v2.0.
</Info>

<Info>
  If you've used the SalamRuby Enterprise Edition with a free beta license key, your instance will be
  downgraded to the Community Edition 2.0. You can find all license details on the [license
  page](https://salamruby.com/docs/self-hosting/license).
</Info>
...
<Warning>
  ...
  salamruby/salamruby:latest` to `image:ghcr.io/salamruby/salamruby:latest` in
...
```bash
docker exec salamruby-postgres-1 pg_dump -Fc -U postgres -d salamruby > salamruby_pre_v2.0_$(date +%Y%m%d_%H%M%S).dump
```
...
```bash
docker pull ghcr.io/salamruby/data-migrations:v2.0.3 && \
docker run --rm \
  --network=salamruby_default \
  -e DATABASE_URL="postgresql://postgres:postgres@postgres:5432/salamruby?schema=public" \
  -e UPGRADE_TO_VERSION="v2.0" \
  ghcr.io/salamruby/data-migrations:v2.0.3
```
...
```ts
import salamruby from "@salamruby/js/website";
...
```ts
import salamruby from "@salamruby/js/app";
...
SalamRuby v1.6 introduces key features like Advanced Targeting, Segmentation, on-the-fly survey triggers, and stability improvements. This update also changes some environment variables. Follow this guide to upgrade your SalamRuby instance to v1.6 smoothly.

<Info>
  This upgrade requires a data migration. Ensure you back up your database before proceeding. Follow the steps
  below to upgrade your SalamRuby instance to v1.6.
</Info>

### Steps to Migrate

This guide is for users self-hosting SalamRuby with our one-click setup. If you're using a different setup, adjust the commands accordingly.

Navigate to the SalamRuby folder containing your `docker-compose.yml` file to run the steps.

- **Backup your Database**: This is a crucial step. Please make sure to backup your database before proceeding with the upgrade. You can use the following command to backup your database:

```bash
docker exec salamruby-quickstart-postgres-1 pg_dump -Fc -U postgres -d salamruby > salamruby_pre_v1.6_$(date +%Y%m%d_%H%M%S).dump
```
...
- Restarting the containers will automatically pull the latest version of SalamRuby:
...
docker run --rm \
  ...
  ghcr.io/salamruby/data-migrations:v1.6.1

```

The above command will migrate your data to the latest schema. This is a crucial step to migrate your existing data to the new structure. Only if the script runs successful, changes are made to the database. The script can safely run multiple times.

1. That's it! Once the migration is complete, you can **now access your SalamRuby instance** at the same URL as before.

#### Restoring the database after a failed upgrade

```bash
docker exec -i salamruby-quickstart-postgres-1 pg_restore --clean -U postgres -v -d salamruby < salamruby_pre_v1.6_<timestamp_of_your_dump_file>.dump
```

<Info>
  Replace the path to `salamruby_pre_v1.6_<timestamp_of_your_dump_file>.dump` with the exact path to your `.dump` file.
</Info>
...
If you're using the `@salamruby/js` package, update it to version \~1.6.5 to access the latest features and improvements.

<Info>
  Currently, the package needs to be pinned to `~1.6.5`, see [this
  issue](https://github.com/salamruby/salamruby/issues/2273).
</Info>

### Upgrade and pin the client package

```bash
npm install @salamruby/js@~1.6.5
```
...
SalamRuby v1.2 introduces new features for Link Surveys and enhances security. However, there are some breaking changes related to environment variables. This guide will help you upgrade your SalamRuby instance to v1.2 smoothly.
...
SalamRuby v1.1 introduces new features and improvements but includes breaking changes with environment variables. This guide will help you upgrade your SalamRuby instance to v1.1 without data loss.
...
| `NEXT_PUBLIC_IS_SALAMRUBY_CLOUD`         | `IS_SALAMRUBY_CLOUD`         |
...
x-environment: &environment
  environment:
    # The url of your SalamRuby instance used in the admin panel
    WEBAPP_URL:

    # Required for next-auth. Should be the same as WEBAPP_URL
    NEXTAUTH_URL:

    # PostgreSQL DB for SalamRuby to connect to
    DATABASE_URL: "postgresql://postgres:postgres@postgres:5432/salamruby?schema=public"
...
If you have any questions or require help, feel free to reach out to us on [**GitHub Discussions**](https://github.com/salamruby/salamruby/discussions). 😃[
](https://salamruby.com/docs/developer-docs/rest-api)
