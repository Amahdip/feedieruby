---
title: "Rate Limiting"
description: "Current request rate limits in SalamRuby"
icon: "timer"
---

SalamRuby applies request rate limits to protect against abuse and keep API usage fair.

Starting with SalamRuby v5, rate limiting is split across two layers:
...
<Warning>
  ...
  protected by Envoy Gateway. If you self-host SalamRuby without Envoy or an equivalent edge rate limiter,
...
Before upgrading to SalamRuby v5:
...
These limits are expected to be enforced at the gateway layer in SalamRuby v5 and later:
...
These are the limits that still run inside the SalamRuby application server:
