...
description: "Learn how authentication and user invitation work in self-hosted SalamRuby deployments."
icon: "user"
---

## Overview

In self-hosted SalamRuby, user management and authentication can be customized using environment variables. By default, self-hosted instances have user signup disabled, and only organization owners or admins can invite new users. The behavior of the authentication and invitation flow can be further controlled using the following environment variables:
...
<Note>
  To control advanced role management features and environment-based SSO behavior, your self-hosted SalamRuby
...
- **Use case:**
  - Set this to `1` if you want to allow anyone with access to your SSO provider to join your SalamRuby instance without a manual invite.
...
<Note>
  SalamRuby does not support special characters, such as Cyrillic, in account email addresses to avoid technical, compatibility, and security issues. Additionally, universal support for such addresses is still limited. 
</Note>