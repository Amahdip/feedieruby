---
title: "CDN"
description: "Use a CDN in front of public SalamRuby traffic without serving stale survey assets after upgrades."
icon: "globe"
---

### Why a CDN is recommended

A CDN is recommended for production self-hosting when you expose SalamRuby surveys publicly. It helps you:
...
You can run SalamRuby without a CDN, but it is usually the better production setup once surveys are reachable from
the public internet.
...
Configure your CDN to proxy requests to your SalamRuby origin while preserving the original `Host` header and HTTPS
...
<Warning>
  Do not enable a blanket "cache everything" rule for all SalamRuby routes. That can cause stale survey pages,
...
"Ignore origin headers". Those features can easily keep old SalamRuby responses alive longer than intended.

## Important detail for upgrades

SalamRuby serves the survey runtime bundle from a stable public path:
...
SalamRuby version, respondents may receive outdated survey code even though the rest of the app has already been
updated.
...
Every time you update SalamRuby, include CDN cache handling in your rollout:

<Steps>
  <Step title="Deploy the new SalamRuby version">
    Update SalamRuby as usual by following the [Migration Guide](/self-hosting/advanced/migration) or your normal
    deployment process.
  </Step>
...
1. Put only the public SalamRuby domain behind the CDN.
...
5. Add a purge step to every SalamRuby deployment or upgrade.
