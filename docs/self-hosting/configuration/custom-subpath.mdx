...
description: "Serve SalamRuby from a custom URL prefix when you cannot expose it on the root domain."
icon: "link"
---
...
service, but you still want SalamRuby to live under the same hostname—for example `https://example.com/feedback`.
Support for a build-time `BASE_PATH` variable is available in the SalamRuby web app so that all internal routes,
...
- Official SalamRuby Docker images do **not** accept this flag for technical reasons, so you must build your own image.
- All public URLs (`WEBAPP_URL`, `NEXTAUTH_URL`, webhook targets, OAuth callbacks, etc.) need the same prefix.
- Your proxy must rewrite `/custom-path/*` to the SalamRuby container while keeping the prefix visible to clients.
...
<Steps>
  <Step title="Clone SalamRuby and prepare secrets">
  ...
  <Step title="Pass BASE_PATH as a build argument">
    Use the SalamRuby web Dockerfile and supply the custom subpath via <code>--build-arg</code>. Example:
...
docker build \
  ...
  --secret id=database_url,src=<(printf "postgresql://user:password@localhost:5432/salamruby?schema=public") \
...
```
  ...
  <Step title="Run the container behind your proxy">
    ...
    <code>http://salamruby-web:3000/custom-path</code> without stripping the prefix.
  </Step>
</Steps>
