...
description: "Configuring your domain for SalamRuby."
icon: "globe"
---

SalamRuby supports both single domain and dual domain configurations with Public/Private Domain Split feature. This guide will help you set up your domains correctly for enhanced security and organization.

## Single Domain Setup

For a single domain setup, you need to configure two essential environment variables:

1. `WEBAPP_URL`: The base URL of your SalamRuby instance (serves both admin and public content)
2. `NEXTAUTH_URL`: The authentication URL (should be the same as WEBAPP_URL)

### Example Configuration

```bash
WEBAPP_URL=https://salamruby.example.com
NEXTAUTH_URL=https://salamruby.example.com
```
...
SalamRuby supports a powerful domain separation feature that allows you to serve public content (surveys, public APIs) from a different domain than your private admin interface. This provides enhanced security and better organization. It enables you to:
...
**Important Security Note**: SalamRuby handles application-level domain restrictions to ensure each domain
...
security must be configured by you. SalamRuby does not manage your infrastructure-level security settings.

### Example Configuration

```bash
# Private admin domain
WEBAPP_URL=https://admin.salamruby.example.com
NEXTAUTH_URL=https://admin.salamruby.example.com
...
3. **Update your reverse proxy** (nginx, Apache, Cloudflare, etc.) to route both domains to your SalamRuby instance
4. **Firewall rules**: Ensure both domains can reach your SalamRuby instance
...
As of now, SalamRuby does not provide an automated migration between domain setups.
