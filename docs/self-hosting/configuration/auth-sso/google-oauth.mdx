---
title: "Google OAuth"
description: "Configure Google OAuth for secure Single Sign-On with your SalamRuby instance. Implement enterprise-grade authentication for your survey platform with Google credentials."
icon: "google"
---

<Note>Google OAuth is part of the SalamRuby [Enterprise Edition](/self-hosting/advanced/license)</Note>

### Google OAuth

Integrating Google OAuth with your SalamRuby instance allows users to log in using their Google credentials, ensuring a secure and streamlined user experience. This guide will walk you through the process of setting up Google OAuth for your SalamRuby instance.

### Requirements

- A Google Cloud Platform (GCP) account

- A SalamRuby instance running

### Account Deletion SSO Confirmation

For SSO-only users, SalamRuby asks the user to type their email address and then redirects them through Google OAuth before deleting the account. SalamRuby asks Google for an interactive login prompt, and the deletion continues only when Google returns the same linked provider account.
...
<Warning>
  If you set `DISABLE_ACCOUNT_DELETION_SSO_CONFIRMATION=1`, SalamRuby skips this SSO identity confirmation
...
### How to connect your SalamRuby instance to Google
...
<Steps>
  ...
  <Step title="Configure OAuth Consent Screen">
    ...
    - Fill in the necessary details and under **Authorized domains**, add the domain where your SalamRuby instance is hosted.
  </Step>
  ...
  <Step title="Restart Your SalamRuby Instance">
    <Note>
      Restarting your Docker containers may cause a brief period of downtime. Plan accordingly.
    </Note>
    
    - Once the environment variables have been updated, it's crucial to restart your Docker containers to apply the changes. This ensures that your SalamRuby instance can utilize the new Google OAuth configuration for user authentication.
