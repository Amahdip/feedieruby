---
title: "Keycloak OIDC"
description: "Configure Keycloak as an OpenID Connect (OIDC) provider for secure Single Sign-On with your self-hosted SalamRuby instance."
icon: "key-skeleton-left-right"
---

<Note>Keycloak OIDC is part of the SalamRuby [Enterprise Edition](/self-hosting/advanced/license)</Note>

Integrating Keycloak with your SalamRuby instance allows users to log in using their existing Keycloak credentials via OpenID Connect. This guide walks you through setting up a Keycloak OIDC client and configuring SalamRuby to use it, including automatic user provisioning into a default team.

### Requirements

- A running Keycloak instance with a configured realm and users.
- A self-hosted SalamRuby instance with a valid [Enterprise license](/self-hosting/advanced/license).
- The SalamRuby callback URI: `{WEBAPP_URL}/api/auth/callback/openid`
...
<Steps>
  <Step title="Create a new client in Keycloak">
    ...
    - Choose a **Client ID** (e.g., `salamruby`) and click **Next**.
  </Step>
  ...
  <Step title="Configure client authentication">
    ...
    <Note>
      SalamRuby uses PKCE (Proof Key for Code Exchange) for added security. Modern Keycloak versions (21+) support PKCE automatically for confidential clients. No additional PKCE configuration is needed.
    </Note>
  </Step>

  <Step title="Set the redirect URI">
    - Under **Valid redirect URIs**, add your SalamRuby callback URI:

    ```
    https://your-salamruby-domain.com/api/auth/callback/openid
    ```

    Replace `your-salamruby-domain.com` with your actual SalamRuby URL.
  ...
  <Step title="Copy the client secret">
    ...
    - Copy the **Client secret** value — you will need this for the SalamRuby configuration.
  </Step>
  ...
  <Step title="Configure SalamRuby environment variables">
    Add the following environment variables to your SalamRuby `.env` file or `docker-compose.yml`:

    ```yml SalamRuby Env for Keycloak OIDC
    OIDC_CLIENT_ID=salamruby
    ...
    | --- | --- |
    | `OIDC_CLIENT_ID` | The Client ID you chose in Keycloak (e.g., `salamruby`). |
    | `OIDC_CLIENT_SECRET` | The client secret from the **Credentials** tab. |
    | `OIDC_ISSUER` | Your Keycloak realm URL (see step above). |
    | `OIDC_DISPLAY_NAME` | The label shown on the SalamRuby login button. |
    | `OIDC_SIGNING_ALGORITHM` | Optional. Keycloak and SalamRuby both default to `RS256`. Only set this if your realm uses a different algorithm. |
  </Step>

  <Step title="Restart and test">
    - Restart your SalamRuby instance.
...
By default, new SSO users must be invited by an organization admin before they can access SalamRuby. To allow Keycloak users to join automatically and be assigned to a team, configure the following additional environment variables:
...
1. Log in to SalamRuby as an admin.
...
<Note>
  The Teams feature requires the **Role-Based Access Control (RBAC)** add-on on your Enterprise license. If you don't see the Teams section in Organization Settings, contact [SalamRuby support](mailto:support@salamruby.com) to enable it.
</Note>
...
2. SalamRuby creates a new user account.
...
<AccordionGroup>
  ...
  <Accordion title="Users see 'No membership found' after login">
    SalamRuby determines the organization automatically from the `AUTH_SSO_DEFAULT_TEAM_ID` — you do not need to set a separate organization ID. If this error appears:
  ...
  <Accordion title="OIDC login button does not appear">
    ...
    - Ensure your SalamRuby instance has a valid Enterprise license.
    - Restart the SalamRuby container after adding the environment variables.
  </Accordion>
  ...
  <Accordion title="Authentication fails with a redirect error">
    ...
    - Check that the OIDC issuer URL is reachable from your SalamRuby server. You can test with:
    ```bash
