...
description: "Using SalamRuby One-Click Setup with a Custom SSL Certificate."
icon: "lock"
---

<Note>
  SalamRuby One-Click setup already comes with a valid SSL certificate using **Let's Encrypt**. This guide is only if you already have a valid SSL certificate that you need to use due to company policy or other requirements.
</Note>

SalamRuby' One-Click setup automatically creates an SSL certificate using **Let's Encrypt**. However, some setups need a custom SSL certificate. This is common for intranets or systems with special certificate rules that use an internal or custom certificate authority (CA).

### Step 1: Navigate to the SalamRuby Folder

Move into the `salamruby/` directory:
  ...
  services:
    salamruby:
      ...
      labels:
        ...
        - "traefik.http.routers.salamruby.rule=Host(`my-domain.com`)"  # Use your actual domain or IP
        - "traefik.http.routers.salamruby.entrypoints=websecure"  # Use the websecure entrypoint (port 443 with TLS)
        - "traefik.http.routers.salamruby.tls=true"  # Enable TLS
        - "traefik.http.services.salamruby.loadbalancer.server.port=3000"  # Forward traffic to SalamRuby on port 3000
      ports:
        - 3000:3000
      volumes:
...
* **Navigate to the SalamRuby folder**
...
This setup ensures that SalamRuby securely communicates using your own SSL certificate. 🚀
