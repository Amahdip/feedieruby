...
description: "Configure file storage for survey images, file uploads, and Workspace assets in your self-hosted SalamRuby instance"
icon: "upload"
---

SalamRuby requires S3-compatible storage for file uploads. You can use external cloud storage services or the bundled RustFS option for a self-hosted solution.

## Why Configure File Uploads?

Setting up file storage enables important features in SalamRuby, including:
...
SalamRuby supports S3-compatible storage with two main configurations:
...
RustFS provides a self-hosted S3-compatible storage solution that runs alongside SalamRuby. This option:

- Runs in a Docker container alongside SalamRuby
...
When using the SalamRuby installation script, you'll be prompted to configure file uploads:
...
Choose this option for a self-hosted S3-compatible storage that runs alongside SalamRuby:
...
- Generate separate RustFS admin and SalamRuby service credentials
- Create the storage bucket
- Configure SSL certificates for the files subdomain
- Configure Traefik routing for the subdomain
- Store the generated RustFS credentials in `./salamruby/.env` with restricted permissions
...
S3_REGION=us-east-1
S3_BUCKET_NAME=my-salamruby-uploads
...
S3_REGION=nyc3
S3_BUCKET_NAME=my-salamruby-space
...
SalamRuby uses the S3 [POST Object](https://docs.aws.amazon.com/AmazonS3/latest/API/RESTObjectPOST.html)
operation (presigned POST) for uploads. Your object storage provider must support this operation. Providers
that do not implement POST Object are not compatible with SalamRuby uploads. For example, Backblaze B2's
S3‑compatible API currently does not support POST Object and therefore will not work with SalamRuby file
uploads.
...
- **Bucket**: Automatically created `salamruby-uploads` bucket
...
🗄️  RustFS Storage Setup Complete:
   • Access Key: salamruby-service-a1b2c3d4
   • Bucket: salamruby-uploads (✅ automatically created)
```
...
services:
  salamruby:
    image: ghcr.io/salamruby/salamruby:latest
    environment:
...
The following IAM policy grants the minimum required permissions for SalamRuby to function correctly. This policy is also used in the bundled RustFS integration:
...
2. **Restricted Write Access**: Only your SalamRuby instance should be able to upload files
3. **CORS Configuration**: Allow requests from your SalamRuby domain
...
```json
{
  "Statement": [
    ...
    },
    {
      ...
      "Effect": "Allow",
      "Principal": {
        "AWS": "arn:aws:iam::123456789012:user/salamruby-service"
      },
      "Resource": "arn:aws:s3:::your-bucket-name/*",
      "Sid": "AllowSalamRubyWrite"
    }
  ],
  "Version": "2012-10-17"
}
...
<Note>
  ...
  `arn:aws:iam::123456789012:user/salamruby-service` with the ARN of your IAM user. This policy allows public
  read access only to specific paths while restricting write access to your SalamRuby service user.
</Note>

### S3 CORS Configuration

CORS (Cross-Origin Resource Sharing) must be configured on your S3 bucket to allow SalamRuby to upload files using presigned POST URLs. Without proper CORS configuration, file uploads from the browser will fail.
...
<Note>
  For production environments, consider restricting `AllowedOrigins` to your specific SalamRuby domain(s)
...
**Bundled SalamRuby setup** (the [one-click script](/self-hosting/setup/one-click), driven by `docker/rustfs-init.sh`): CORS is configured for you. The bootstrap applies a per-bucket policy via `mc cors set`, scoped to your SalamRuby domain — nothing else to do.

**Standalone RustFS** (managed separately from SalamRuby): set `RUSTFS_CORS_ALLOWED_ORIGINS` on the RustFS server to the origin(s) you serve SalamRuby from — your `WEBAPP_URL` — comma-separated for multiple values:
...
<Note>
  ...
  this variable is most likely already set to a value that excludes your SalamRuby origin — add your origin to it
...
- Ship RustFS and SalamRuby container logs into your normal logging and alerting stack; the bundled setup
...
5. We use S3 presigned URLs for uploads. Make sure your CORS policy allows presigned URL uploads; otherwise, uploads will fail.
   Some providers (e.g., Hetzner’s object storage) [require a specific CORS configuration](https://github.com/salamruby/salamruby/discussions/6641#discussioncomment-14574048).
   If you’re using the bundled RustFS setup, this is already configured for you.
6. **The file appears in your storage but SalamRuby still reports “upload failed.”** This is the classic CORS symptom — the browser uploaded the file successfully but could not read the response. For RustFS, set `RUSTFS_CORS_ALLOWED_ORIGINS` to include your SalamRuby origin (see [RustFS CORS Configuration](#rustfs-cors-configuration)).
...
```bash
# Check SalamRuby logs
docker compose logs salamruby

# Check RustFS logs (if using bundled RustFS)
docker compose logs rustfs
```

For additional help, join the conversation on [GitHub Discussions](https://github.com/salamruby/salamruby/discussions).
