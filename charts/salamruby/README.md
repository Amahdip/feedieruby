...
A Helm chart for SalamRuby with PostgreSQL, Valkey

**Homepage:** <https://salamruby.com/docs/self-hosting/setup/kubernetes>

## Maintainers

| Name       | Email                 | Url |
| ---------- | --------------------- | --- |
| SalamRuby | <info@salamruby.com> |     |
...
The chart can optionally deploy SalamRuby behind Envoy Gateway with a dedicated Redis HA backend for Envoy global
rate limiting.
...
- OpenTelemetry proxy metrics can target either `envoy.salamruby.proxy.telemetry.openTelemetry.host`/`port`
  or `backendRefs`, but not both. Prefer `host`/`port` for collectors that live in another namespace.
- When both the main app ingress and the Envoy API ingress are enabled, set `envoy.salamruby.ingress.host`
...
Cube is part of the baseline SalamRuby v5 stack and is deployed by this chart by default
(`cube.enabled: true`).

- For the chart-managed Cube, the chart renders `deployment.env.CUBEJS_API_URL` automatically as
  `http://salamruby-cube:4000` when using the default release name.
...
When the SalamRuby migration job is enabled, Hub waits for the `salamruby-migration` Job to complete before its own goose/river init migrations run. This keeps fresh shared-database installs from creating Hub tables before Prisma has initialized the SalamRuby schema.
...
When deployed with Argo CD, chart-managed Secrets and ExternalSecrets render in sync wave `-2`, and the SalamRuby and Hub migration hooks run in sync wave `-1`. This lets app and Hub secrets exist before migration jobs start.
...
The chart can optionally deploy a SalamRuby-provided Qwen runtime through the `vllm-stack` dependency. It is disabled by default so existing installs keep using their current AI provider settings.

To deploy the bundled Qwen/vLLM runtime and automatically point the SalamRuby app at it:
...
Set `llm.autoConfigureApp=false` to deploy the bundled runtime without injecting SalamRuby app AI env vars.
...
| deployment.image.repository                                        | string | `"ghcr.io/salamruby/salamruby"`                                           |                                                           |
...
| salamruby.publicUrl                                               | string | `""`                                                                        |                                                           |
| salamruby.webappUrl                                               | string | `""`                                                                        |                                                           |
...
| hub.migration.waitForSalamRubyMigration.enabled                   | bool   | `true`                                                                      |                                                           |
| hub.migration.waitForSalamRubyMigration.intervalSeconds           | int    | `5`                                                                         |                                                           |
| hub.migration.waitForSalamRubyMigration.maxAttempts               | int    | `180`                                                                       |                                                           |
| hub.migration.waitForSalamRubyMigration.missingJobMaxAttempts     | int    | `12`                                                                        | Consecutive missing Job reads before using DB markers.    |
...
| ingress.hosts[0].host                                              | string | `"k8s.salamruby.com"`                                                      |                                                           |
| ingress.hosts[0].paths[0].path                                     | string | `"/"`                                                                       |                                                           |
| ingress.hosts[0].paths[0].pathType                                 | string | `"Prefix"`                                                                  |                                                           |
| ingress.hosts[0].paths[0].serviceName                              | string | `"salamruby"`                                                              |                                                           |
...
| llm.salamruby.baseUrl                                             | string | `""`                                                                        | Defaults to `http://<release-name>-router-service:<llm.routerSpec.servicePort>/v1`. |
| llm.salamruby.model                                               | string | `"qwen3-14b-awq"`                                                           | SalamRuby `AI_MODEL` value for the bundled runtime.      |
| llm.salamruby.providerName                                        | string | `"vllm"`                                                                    | SalamRuby OpenAI-compatible provider display name.       |
| llm.salamruby.supportsStructuredOutputs                           | string | `"1"`                                                                       | Enables structured output usage for the bundled runtime.  |
...
| postgresql.auth.database                                           | string | `"salamruby"`                                                              |                                                           |
| postgresql.auth.existingSecret                                     | string | `"salamruby-app-secrets"`                                                  |                                                           |
...
| postgresql.auth.username                                           | string | `"salamruby"`                                                              |                                                           |
| postgresql.enabled                                                 | bool   | `true`                                                                      |                                                           |
| postgresql.externalDatabaseUrl                                     | string | `""`                                                                        |                                                           |
| postgresql.fullnameOverride                                        | string | `"salamruby-postgresql"`                                                   |                                                           |
...
| redis.auth.existingSecret                                          | string | `"salamruby-app-secrets"`                                                  |                                                           |
| redis.auth.existingSecretPasswordKey                               | string | `"REDIS_PASSWORD"`                                                          |                                                           |
| redis.enabled                                                      | bool   | `true`                                                                      |                                                           |
| redis.externalRedisUrl                                             | string | `""`                                                                        |                                                           |
| redis.fullnameOverride                                             | string | `"salamruby-redis"`                                                        |                                                           |
