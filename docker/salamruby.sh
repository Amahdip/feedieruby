...
# source template for the generated rustfs-init.sh in docker/salamruby.sh for
...
if [ -n "${RUSTFS_CORS_ALLOWED_ORIGINS:-}" ]; then
  ...
  cors_file="/tmp/salamruby-cors.xml"
...
cat > /tmp/salamruby-policy.json << EOF
{
  "Version": "2012-10-17",
  "Statement": [
    {
...
if ! mc admin policy info rustfs "$RUSTFS_POLICY_NAME" >/dev/null 2>&1; then
  mc admin policy create rustfs "$RUSTFS_POLICY_NAME" /tmp/salamruby-policy.json || \
    mc admin policy add rustfs "$RUSTFS_POLICY_NAME" /tmp/salamruby-policy.json
...
write_rustfs_env_file() {
  ...
  upsert_dotenv_var "SALAMRUBY_RUSTFS_ADMIN_USER" "$rustfs_admin_user" "$env_file"
  upsert_dotenv_var "SALAMRUBY_RUSTFS_ADMIN_PASSWORD" "$rustfs_admin_password" "$env_file"
  upsert_dotenv_var "SALAMRUBY_RUSTFS_SERVICE_USER" "$rustfs_service_user" "$env_file"
  upsert_dotenv_var "SALAMRUBY_RUSTFS_SERVICE_PASSWORD" "$rustfs_service_password" "$env_file"
  upsert_dotenv_var "SALAMRUBY_RUSTFS_BUCKET_NAME" "$rustfs_bucket_name" "$env_file"
  upsert_dotenv_var "SALAMRUBY_RUSTFS_POLICY_NAME" "$rustfs_policy_name" "$env_file"
  upsert_dotenv_var "SALAMRUBY_RUSTFS_REGION" "us-east-1" "$env_file"
}

install_salamruby() {
  # Friendly welcome
  echo "🧱 Welcome to the SalamRuby Setup Script"
  echo ""
  echo "🛸 Fasten your seatbelts! We're setting up your SalamRuby environment on your $ubuntu_version server."
  echo ""
  ...
  echo "🎉 Hooray! Docker is all set and ready to go. You're now ready to run your SalamRuby instance!"

  mkdir -p salamruby && cd salamruby
  echo "📁 Created SalamRuby Quickstart directory at ./salamruby."
  ...
  if [[ $https_setup == "n" ]]; then
    echo "You have chosen not to set up HTTPS certificate for your domain. Please make sure to set up HTTPS on your own. You can refer to the SalamRuby documentation(https://salamruby.com/docs/self-hosting/custom-ssl) for more information."
...
EOT
  ...
  echo "📥 Downloading docker-compose.yml from SalamRuby GitHub repository..."
  curl -fsSL -o docker-compose.yml https://raw.githubusercontent.com/salamruby/salamruby/stable/docker/docker-compose.yml
  mkdir -p cube/schema
  echo "📥 Downloading Cube.js configuration for XM Suite v5 analytics..."
  curl -fsSL -o cube/cube.js https://raw.githubusercontent.com/salamruby/salamruby/stable/docker/cube/cube.js
  curl -fsSL -o cube/schema/FeedbackRecords.js https://raw.githubusercontent.com/salamruby/salamruby/stable/docker/cube/schema/FeedbackRecords.js
...
EOF
  ...
  # Step 1: Add Traefik labels to salamruby service
  awk -v domain_name="$domain_name" -v hsts_enabled="$hsts_enabled" '
/salamruby:/,/^ *$/ {
    if ($0 ~ /<<: \*environment$/) {
        print "    labels:"
        print "      - \"traefik.enable=true\""
        print "      - \"traefik.http.routers.salamruby.rule=Host(`" domain_name "`)\""
        print "      - \"traefik.http.routers.salamruby.entrypoints=websecure\""
        print "      - \"traefik.http.routers.salamruby.tls=true\""
        print "      - \"traefik.http.routers.salamruby.tls.certresolver=default\""
        print "      - \"traefik.http.services.salamruby.loadbalancer.server.port=3000\""
        ...
        print "      - \"traefik.http.routers.feedback-records-token.service=salamruby\""
        ...
        if (hsts_enabled == "y") {
            ...
            print "      - \"traefik.http.routers.salamruby_http.entrypoints=web\""
            print "      - \"traefik.http.routers.salamruby_http.rule=Host(`" domain_name "`)\""
            ...
            print "      - \"traefik.http.routers.feedback-records-token-http.service=salamruby\""
...
in_hub && /^  [A-Za-z0-9_-]+:/ && !/^  hub:/ { in_hub = 0 }
{
    if (in_hub && !inserted && $0 ~ /^    environment:/) {
        ...
        print "      - \"traefik.http.middlewares.feedback-records-auth.forwardauth.address=http://salamruby:3000/api/traefik-auth/feedback-records\""
...
' docker-compose.yml >tmp.yml && mv tmp.yml docker-compose.yml
  ...
  # Step 2: Ensure salamruby waits for rustfs-init to complete successfully (mapping depends_on)
...
echo "🔗 To edit more variables and deeper config, go to the salamruby/docker-compose.yml, edit the file, and restart the container!"
...
if [[ $rustfs_storage == "y" ]]; then
    ...
    echo "   • Generated credentials stored in ./salamruby/.env (permissions set to 600)"
    echo ""
fi

echo "🎉 All done! Please setup your SalamRuby instance by visiting your domain at https://$domain_name. You can check the status of SalamRuby & Traefik with 'cd salamruby && sudo docker compose ps.'"
...
uninstall_salamruby() {
  echo "🗑️ Preparing to Uninstalling SalamRuby..."
  read -p "Are you sure you want to uninstall SalamRuby? This will delete all the data associated with it! (yes/no): " uninstall_confirmation
  uninstall_confirmation=$(echo "$uninstall_confirmation" | tr '[:upper:]' '[:lower:]')
  if [[ $uninstall_confirmation == "yes" ]]; then
    cd salamruby
    sudo docker compose down
    cd ..
    sudo rm -rf salamruby
    echo "🛑 SalamRuby uninstalled successfully!"
  else
    echo "❌ Uninstalling SalamRuby has been cancelled."
  fi
}

stop_salamruby() {
  echo "🛑 Stopping SalamRuby..."
  cd salamruby
  sudo docker compose down
  echo "🎉 SalamRuby instance stopped successfully!"
}

update_salamruby() {
  echo "🔄 Updating SalamRuby..."
  cd salamruby
  sudo docker compose pull
  sudo docker compose down
  sudo docker compose up -d
  echo "🎉 SalamRuby updated successfully!"
  echo "🎉 Check the status of SalamRuby & Traefik with 'cd salamruby && sudo docker compose logs.'"
}

restart_salamruby() {
  echo "🔄 Restarting SalamRuby..."
  cd salamruby
  sudo docker compose restart
  echo "🎉 SalamRuby restarted successfully!"
}

get_logs() {
  echo "📃 Getting SalamRuby logs..."
  cd salamruby
...
if [[ "${BASH_SOURCE[0]}" == "$0" ]]; then
  ...
  uninstall)
    ...
    ;;
  *)
    echo "🚀 Executing default step of installing SalamRuby"
    install_salamruby
    ;;
  esac
fi
